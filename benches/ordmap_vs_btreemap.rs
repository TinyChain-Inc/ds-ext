...
fn benchmark_list_vs_vec(c: &mut Criterion) {
    ...
    for size in [10, 100, 1_000, 10_000].iter() {
        let mut map = OrdHashMap::from_iter((0..*size).map(|i| (i, i)));
        group.bench_with_input(BenchmarkId::new("OrdHashMap", size), size, |b, _i| {
            b.iter(|| ord_read(&mut map, 10_000))
        });

        let mut map = BTreeMap::from_iter((0..*size).map(|i| (i, i)));
        group.bench_with_input(BenchmarkId::new("BTreeMap", size), size, |b, _i| {
